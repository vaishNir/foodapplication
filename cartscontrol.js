...
const addcart = async (req, res) => {
  try {
    ...
    if (!foodid || !CustomerId || !count) {
      return res.status(400).json({message: "All fields are required"})
    }
  
    const newCart = new cartmodel ({
      foodid, CustomerId,count
    })
    await newCart.save();
    return res.status(200).json({message: "Cart added ", data: newCart})
  } catch (error) {
...
//     await cartmodel
//       .findOneAndUpdate(
//         { foodid: req.params.foodid, userid: req.body.userid },
//         { count: count + parseInt(req.body.count) }
//       )
//       .exec()
//       .then((datas) => {
//         console.log("updated");
...
//       status: 500,
//       msg:
//         "Already added to cart!! Count of Food updated to " +
...
const viewcart = (req, res) => {
  ...
  console.log("cc id",req.params.CustomerId);
  cartmodel
    .find({ CustomerId })
    .populate("foodid").populate("CustomerId") // Filtering based on customerId
    .exec()
    .then((data) => {
